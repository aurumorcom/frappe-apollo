...
- **API Key & OAuth Tokens**: Stored in `Apollo Account` DocType. Secret tokens (`client_secret`, `access_token`, `refresh_token`, `webhook_bearer_token`) use Frappe's `Password` fieldtype ([`apps/frappe_apollo/frappe_apollo/apollo/doctype/apollo_account/apollo_account.json`](apps/frappe_apollo/frappe_apollo/apollo/doctype/apollo_account/apollo_account.json:38)).
