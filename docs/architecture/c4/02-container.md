...
| `Worker FS` | Redis Streams, FastStream & Bench Workers | Asynchronously processes background tasks like sequence provisioning, field mapping, and webhook payloads via `frappe.enqueue()` and `frappe_controller.utils.background_jobs.enqueue()` ([`apps/frappe_apollo/frappe_apollo/apollo/doctype/cadence/cadence.py`](apps/frappe_apollo/frappe_apollo/apollo/doctype/cadence/cadence.py:3)). |
