...
class TestCadenceProvisioningExternal(IntegrationTestCase):
	...
	def setUp(self):
		...
		if self.account_name == "Dummy VCR Account" and not frappe.db.exists(
			"Apollo Account", self.account_name
		):
			frappe.get_doc(
				{
		...
		frappe.db.set_value(
			"Apollo Account", self.account_name, "apollo_sequence_id", "6a5aecd3bcbdfc0020ac5853"
		)
	...
	def test_create_cadence(self):
		...
		if not frappe.db.exists("Email Template", "Test Template"):
			frappe.get_doc(
				{"doctype": "Email Template", "name": "Test Template", "subject": "Test", "response": "Test"}
		...
		frappe.get_doc(
			{
				...
				"apollo_ids": [{"account": self.account_name, "sender": "Administrator"}],
			}
		).insert(ignore_permissions=True, ignore_mandatory=True)
