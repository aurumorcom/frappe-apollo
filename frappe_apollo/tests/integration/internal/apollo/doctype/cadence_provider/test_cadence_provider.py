...
class TestCadenceProvider(IntegrationTestCase):
	...
	def test_mcc_sequence_assignment_suspends_when_provider_disabled(
	...
	def test_cadence_provider_on_update_enqueues_valid_method_path(self, mock_enqueue, mock_get_all):
		...
		method_path = (
			mock_enqueue.call_args[1].get("method")
			if mock_enqueue.call_args[1].get("method")
			else mock_enqueue.call_args[0][0]
		)
		self.assertEqual(method_path, "frappe_apollo.apollo.doctype.cadence.cadence.update_sequence_steps")
