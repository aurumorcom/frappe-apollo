...
class TestWebhookIntegration(IntegrationTestCase):
	def setUp(self):
		...
		lead = frappe.get_doc({"doctype": "CRM Lead", "first_name": "Webhook", "email": lead_email}).insert(
			ignore_permissions=True
		)

		account = frappe.get_doc(
			{"doctype": "Apollo Account", "account_name": account_name, "webhook_bearer_token": "valid_token"}
		).insert(ignore_permissions=True)

		lead.append("apollo_ids", {"account": account.account_name, "apollo_id": "contact_123"})
		lead.save(ignore_permissions=True)

		cadence = frappe.get_doc({"doctype": "Cadence", "cadence_name": cadence_name}).insert(
			ignore_permissions=True
		)
	...
	def test_security_authorized(self, mock_header, mock_enqueue):
		...
		mock_enqueue.assert_called_once_with(
			method="frappe_apollo.webhook.process_webhook", queue="low", payload={"event": "test"}
		)
	...
	def test_process_webhook_message_sent(self, mock_report_event):
		...
		payload = {"event": "message_sent", "contact_id": "contact_123", "emailer_campaign_id": "seq_123"}
		...
		mock_report_event.assert_called_once_with(
			"message_sent", {"mcc_name": self.mcc_name, "communication_name": comm.name}, payload
		)
	...
	def test_process_webhook_message_opened(self, mock_report_event):
		...
		payload = {"event": "message_opened", "contact_id": "contact_123", "emailer_campaign_id": "seq_123"}
		...
		mock_report_event.assert_called_once_with(
			"message_opened", {"mcc_name": self.mcc_name, "communication_name": comm.name}, payload
		)

	@patch("frappe_apollo.webhook.report_event")
	def test_process_webhook_message_replied(self, mock_report_event):
		payload = {"event": "message_replied", "contact_id": "contact_123", "emailer_campaign_id": "seq_123"}
		...
		mock_report_event.assert_called_once_with("message_replied", {"mcc_name": self.mcc_name}, payload)

	@patch("frappe_apollo.webhook.report_event")
	def test_process_webhook_unmapped(self, mock_report_event):
		payload = {"event": "message_sent", "contact_id": "unknown", "emailer_campaign_id": "seq_123"}
